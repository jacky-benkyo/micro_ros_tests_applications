...
// Include the TDD decoupled logic module
...
void init_gy801() {
    ...
    // Start ADXL345： Write 0x08 to 0x2D register (Measurement Mode)
...
void timer_callback(rcl_timer_t *timer, int64_t last_call_time) {
    ...
    // Peripheral IO
    uint8_t reg_accel = ADXL345_REG_DATA;
    i2c_write_blocking(I2C_PORT, ADXL345_ADDR, &reg_accel, 1, true);
    i2c_read_blocking(I2C_PORT, ADXL345_ADDR, accel_data, 6, false);

    uint8_t reg_gyro = GYRO_REG_DATA | 0x80;
    i2c_write_blocking(I2C_PORT, GYRO_ADDR, &reg_gyro, 1, true);
    i2c_read_blocking(I2C_PORT, GYRO_ADDR, gyro_data, 6, false);

    // Call the unit-tested decoupled math logic
    imu_update_pitch(&my_global_processor, accel_data, gyro_data, dt);
    
    // Fill ROS 2 message using tested logic
    imu_get_quaternion(my_global_processor.angle_pitch, &msg.orientation.w, &msg.orientation.x, &msg.orientation.y, &msg.orientation.z);
...
int main()
{
    ...
    // Initialize the unit-tested controller module configuration
    imu_init(&my_global_processor, ALPHA);
